...
def run_inference():
    ...
    # 4. Recursive Forecasting (Bridge to Now + Next 24 Hours) — per station
    now_utc = datetime.now(pytz.UTC)

    station_info = df[['station_id', 'station_rank']].drop_duplicates().sort_values('station_rank')
    print(f" Found {len(station_info)} stations: {station_info['station_id'].tolist()}")

    predictions = []

    for _, station_row in station_info.iterrows():
        sid = station_row['station_id']
        srank = int(station_row['station_rank'])

        station_df = df[df['station_id'] == sid].sort_values('start_hour')
        last_known_hour = station_df['start_hour'].max()
        ...
        print(f" Station {srank} ({sid}): bridging {hours_to_now}h gap from {last_known_hour}...")

        current_state = station_df.iloc[-1].copy()
        ...
        # Step B: Generate the actual 24h forecast for this station
    ...
    print(f" Generated {len(predictions)} predictions across {len(station_info)} stations.")
