    ...
    if not os.path.exists(zip_path) and not csv_exists:
        ...
        for url in urls:
            try:
                ...
                continue
        if not success:
            print(f" Could not find data for {ym}")
            return None
...
def generate_monthly_stats(ym, bucket):
    ...
    if not csv_path:
        print(f" Could not download {ym} for monthly stats. Skipping.")
        return
    ...
    try:
        ...
        print(f" Column mismatch, loading available columns: {e}")
    ...
    if bucket:
        ...
        print(" AWS_S3_BUCKET not set. Skipping S3 upload for monthly stats.")
...
def run_pipeline():
    ...
    print(" Finalizing Global Aggregates...")
